...
exports.findAllByDeviceId = (req, res) => {
  Activity.find({
    ...
    .then(activities => {
      if (!activities) {
        return res.status(404).send({
          message:
            "Activity updates not found for device id: " + req.params.device_id
        });
      }
      res.send(activities);
    })
    .catch(err => {
      ...
      return res.status(500).send({
        message:
          "Error retrieving activity updates for device id: " +
  ...
  Activity.findOne({
    ...
    .then(activity => {
      if (!activity) {
        return res.status(404).send({
          message:
            "No last activity found for device id: " + req.params.device_id
        });
      }
      res.send(activity);
    })
    .catch(err => {
      ...
      return res.status(500).send({
        message:
          "Error retrieving last activity for device id: " +
          req.params.device_id
      });
    });
};
