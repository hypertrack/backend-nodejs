...
exports.findAll = (req, res) => {
  Health.find()
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving health updates."
      });
    });
};
...
exports.findAllByDeviceId = (req, res) => {
  Health.find({
    ...
    .then(health => {
      if (!health) {
        return res.status(404).send({
          message:
            "Health updates not found for device id: " + req.params.device_id
        });
      }
      res.send(health);
    })
    .catch(err => {
      ...
      return res.status(500).send({
        message:
          "Error retrieving health updates for device id: " +
  ...
  Health.findOne({
    ...
    .then(health => {
      if (!health) {
        return res.status(404).send({
          message:
            "No last health update found for device id: " + req.params.device_id
        });
      }
      res.send(health);
    })
    .catch(err => {
      ...
      return res.status(500).send({
        message:
          "Error retrieving last health update for device id: " +
          req.params.device_id
      });
    });
};
