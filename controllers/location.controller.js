...
exports.findAllByDeviceId = (req, res) => {
    Location
        ...
        .then(locations => {
            if (!locations) {
                return res.status(404).send({
                    message: "Location updates not found for device id: " + req.params.device_id
                });
            }
            res.send(locations);
        })
        .catch(err => {
            ...
            return res.status(500).send({
                message: "Error retrieving location updates for device id: " + req.params.device_id
            });
        });
};
...
exports.findLastByDeviceId = (req, res) => {
    Location
        ...
        .then(location => {
            if (!location) {
                return res.status(404).send({
                    message: "No last location found for device id: " + req.params.device_id
                });
            }
            res.send(location);
        })
        .catch(err => {
            ...
            return res.status(500).send({
                message: "Error retrieving last location for device id: " + req.params.device_id
            });
        });
};